...
#include <com_yyh_fork_NativeRuntime.h>
#include <string.h>
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
...
int start(int argc, char* srvname, char* sd) {
	...
	if (pid < 0) {
		...
		if (pid == 0) { // 第二个子进程
			...
			umask(0);
			ret = pthread_create(&id, NULL, thread, srvname);
...
int strstr_Wrapper(const char* haystack, const char* needle,
	...
	if (intCaseSensitive) {
		char* chars = strstr(haystack, needle);

		return atoi(chars);
	}else{
		char* charss = strcasestr(haystack, needle);
		return atoi(charss);
	}
}
 ...
 * JNIEXPORT
 */
jint JNICALL Java_com_yyh_fork_NativeRuntime_stopService(JNIEnv *env, jobject thiz) {
	exit(0);
	jint inta  =1;
	return inta;
}
